...
function UpdatePersonal({list}) {
    const [employeeNumber, setEmployeeNumber] = useState(list.employeeNumber);
    ...
    const [Cnic, setCnic] = useState(list.Cnic);
    const [fatherName, setFatherName] = useState(list.fatherName);
    const [designation,setDesignation] = useState(list.designation);
    const [salary,setSalary]=useState(list.salary)
    const [activeStatus,setActiveStatus]=useState(list.activeStatus);
    const [joiningDate,setJoiningDate]=useState(list.joiningDate);
    const [remark,setremark]=useState(list.remark);
    const [profile,setprofile]=useState(list.profile);
    const [loading,setLoading]=useState(false);
    const [prejoiningDocuments,setprejoiningDocuments]=useState(list.prejoiningDocuments);
    const [joiningDocuments,setjoiningDocuments]=useState(list.joiningDocuments);
    const [serviceHistory,setserviceHistory]=useState(list.serviceHistory);
...
const handleSubmit = (e) => {
    ...
    try {
        setLoading(true);
        axios.post('http://localhost:5000/api/personal/update', 
        {
            employeeNumber,
            firstName,
            lastName,
    ...
    return ( 
        <>
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target=".bd-example-modal-xl3">Update Profile</button>
   ...
   <div class="modal-dialog modal-xl modal-dialog-scrollable" role="document">
     <div class="modal-content">
       ...
       <div class="modal-header">
         <h5 class="modal-title h4" id="myExtraLargeModalLabel">Update Personal Profile</h5>
                        ...
                        <>
                            {
                                consfirmationReprotMessage?
                                    ...
                                    <div class="col-sm-5">
                                    <input type="file" class="js-file-attach form-control"  onChange={handleProfile}  disabled={consfirmationReprotMessage?true:false} />
                                    </div>
